...
function shop_view(id) {
    ...
    var sendInfo = {
        ...
        "action": "retrieve",
        id:"id",
        "from_date_1": $('#from_date_1').val(),
        "to_date_1": $('#to_date_1').val(),
        "group_id": $("#group_id").val(),
        "item_name_1": $("#item_name_1").val()
    };
    ...
    $.ajax({
        ...
        data: sendInfo,
        success: function (data) {
            // Assuming that data contains HTML with the shop name
            $('#bd-example-modal-lg1').modal('show');
            $('#bd-example-modal-lg1 #model_main_content').html(data);

            // Get the shop name from the modal content and display it
            var shopName = $('#bd-example-modal-lg1 #shopName').text();
