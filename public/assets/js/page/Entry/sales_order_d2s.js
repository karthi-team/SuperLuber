function filter(from_date, to_date,sales_exec_1,dealer_creation_id_1 ,order_no_1, ) {
    ...
    var sendInfo = {
        _token:_token,"action": "retrieve", "from_date": f_date, "to_date": t_date,"sales_exec":sales_exec_1 ,"dealer_creation_id": dealer_creation_id_1, "order_no_1": order_no_1 ,"user_rights_edit_1": user_rights_edit_1, "user_rights_delete_1": user_rights_delete_1
    };
    $.ajax({
        type: "POST",
        url: $("#CUR_ACTION").val(),
...
function insert_update_row(
    ...
    //     const market_idSelect = document.getElementById('market_creation_id');
    //   const selectedmarket_id = [];
    //   for (let i = 0; i < market_idSelect.options.length; i++) {
    //     if (market_idSelect.options[i].selected) {
    //         selectedmarket_id.push(market_idSelect.options[i].value);
    //     }
    //   }
    //   var market_id_sub = selectedmarket_id.join(',');

      var selectedmarket_id = [];
  var market_idSelect = document.getElementsByClassName('market_creation_id');
  for (let i1 = 0; i1 < market_idSelect.length; i1++) {
    selectedmarket_id.push(market_idSelect[i1].value);
  }
  var market_id_sub = market_idSelect;
        if (id == "") {
            var _token=$("#_token").val();
            var sendInfo = {
                _token:_token,
                action: "insert",
                order_date: order_date,
                market_creation_id: market_id_sub,
                dealer_creation_id: dealer_creation_id,
...
function visitors_insert_update_row(id,d2s_id,order_date,order_no,visitor_dis,visitor_name,sales_exec,mobile_no,description,address,dealer_id)
{
  var _token=$("#_token").val();
  if(id=="")
  {
        ...
        formData.append("visitor_dis", visitor_dis);
...
function getordernumber_admin(){
    var dealer_creation_id = $("#dealer_creation_id_1").val();
    var _token=$("#_token").val();
    var sendInfo = {
        "_token":_token,
