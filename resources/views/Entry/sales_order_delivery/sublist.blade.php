...
<table class="table table-sm table-hover" style="width:100%;">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <input type="date" id="order_date_sub" name="order_date_sub" class="form-control order_date_sub" style="width: auto;" value="<?php if(isset($value_date)){ echo $value_date;}else{ echo $sales_order_delivery_sub_list1['order_date_sub'];} ?>" readonly>
            </td>
            ...
            <td>
                <input type="number" min="1" class="form-control balance_quantity" id="balance_quantity_<?php echo $i1; ?>" style="width: auto;" value="<?php echo $sales_order_delivery_sub_list1['balance_quantity']; ?>" readonly/>
            </td>
            <td>
                <input type="text" min="1" class="form-control return_quantity" id="return_quantity_<?php echo $i1; ?>" style="width: auto;" value="<?php if($sales_order_delivery_sub_list1['return_quantity']!=''){ echo $sales_order_delivery_sub_list1['return_quantity']; }else{ echo "0"; } ?>" oninput="calc_total_amount('<?php echo $i1; ?>');balance_validation('<?php echo $i1; ?>');"/>
                <div id="return_quantity_validate_div_<?php echo $i1; ?>" class="mark_label_red"></div>
            </td>
            <td>
...
function perm1_function(checkbox, rowIndex) {
    ...
    var balanceQuantities = document.querySelectorAll('.balance_quantity');
    var returnQuantities = document.querySelectorAll('.return_quantity');

    for (var i = 0; i < balanceQuantities.length; i++) {
        var balanceQuantity = balanceQuantities[i].value;
        var returnQuantity = returnQuantities[i];
        if (returnQuantity) {
            var return_id = returnQuantities[i].id;
            var match = return_id.match(/\d+/);
            var numericValue = match ? parseInt(match[0], 10) : 0;
            calc_total_amount(numericValue);
            returnQuantity.value = checked ? balanceQuantity : "0";
        }
    }
    });
}
...
function balance_validation(id)
{
    var balance_quantity=$("#balance_quantity_"+id).val();
    balance_quantity=(balance_quantity!="")?parseFloat(balance_quantity):0;
    var return_quantity=$("#return_quantity_"+id).val();
    return_quantity=(return_quantity!="")?parseFloat(return_quantity):0;
    if(balance_quantity < return_quantity){
        $("#return_quantity_"+id).addClass("is-invalid");
        $("#return_quantity_validate_div_"+id).html("Check Pending Quantity");
        return false;
    } else {
        $("#return_quantity_"+id).removeClass("is-invalid");
        $("#return_quantity_validate_div_"+id).html("");
    }
}
</script>
