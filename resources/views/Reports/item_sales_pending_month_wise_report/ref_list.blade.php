    ...
    <table class="table table-hover" id="tableExport" border="1">
        <thead>
            <tr style="border-bottom: 1px solid black">
                ...
                if ($manager_id != '') {
                    foreach ($group_list as $i1 => $group_list1) {
                        if (count($group_list1['item_list']) > 0) {
                            $total_col1 += $ch_dispatch == 1 && $ch_pendings == 1 ? 2 : 1;
                        }
                    }
                    if ($ch_dispatch == 1) {
                        $total_col1 += 1;
                ...
                <th colspan="{{ $total_col1 }}" style="color: black;text-align:left;"><strong>Sales Ref Name :
                    </strong>{{ $sales_rep_detail }}</th>
            </tr>
            <tr>
                <th rowspan="2" style="width:50px;border-bottom: 1px solid black;background-color: #cfcfcfad;">S.NO
                </th>
                <th rowspan="2" style="width:150px;border-bottom: 1px solid black;background-color: #cfcfcfad;">
                    Dealer Name</th>
                <th rowspan="2" style="width:150px;border-bottom: 1px solid black;background-color: #cfcfcfad;">
                    Dealer Address</th>
                @if ($manager_id != '')
            ...
            <tbody>
                ...
                @foreach ($sales_ref_list as $index1 => $sales_ref_list1)
                    @foreach ($sales_ref_list1['dealer_list'] as $index2 => $dealer_list1)
                        ...
                        $string .= "<td style=\"text-align:left;width:150px;\">$dealer_list1[dealer_name]</td>";
                        $string .= "<td style=\"text-align:left;width:150px;\">$dealer_list1[place]</td>";
                        $col = 0;
                        foreach ($group_list as $i1 => $group_list1) {
                            if (count($group_list1['item_list']) > 0) {
                                if ($ch_dispatch == 1) {
