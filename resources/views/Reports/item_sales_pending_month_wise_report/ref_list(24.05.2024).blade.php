    ...
    <table class="table table-hover" id="tableExport" border="1">
        <thead>
            <tr style="border-bottom: 1px solid black">
                ...
                if ($manager_id != ''){
                    foreach ($group_list as $i1 => $group_list1){
                        if (count($group_list1['item_list']) > 0){
                            $total_col1 += (count($group_list1['item_list']) * ($ch_dispatch == 1 && $ch_pendings == 1 ? 2 : 1));
                        }
                    }
                    if ($ch_dispatch == 1){
                        $total_col1 += 1;
                ...
                <th colspan="{{ $total_col1 }}" style="color: black;"><strong>Sales Ref Name : </strong>{{ $sales_rep_detail }}</th>
            </tr>
            <tr>
