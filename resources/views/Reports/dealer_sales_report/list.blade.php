...
<div class="table-responsive"><br>
    <table class="table table-hover" id="tableExport" style="width:100%;" border="1">
        <thead>
            <tr>
                <th colspan="6">
                    <center>Dealer Sale</center>
                </th>
            </tr>
            <tr style="border-bottom: 1px solid black !important;">
                <th colspan="6">
                    <center>Dealer - {{$name_deal}} {{$adress}}</center>
                </th>
            </tr>
            <tr>
                <th>Date</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td id="targetTd">
                    ...
                    @php
                    $total_opening_stock_pass = 0 ;
                    $total_opening_stock_pass = $opens->total_opening_stock;
                    $openCount++;
                ...
                <td>
                    ...
                    @php
                    $total_order_quantity_pass = 0;
                    $total_order_quantity_pass = $orders->total_order_quantity;
                    $orderCount++;
                ...
                <td>
                    ...
                    @if ($openning_date == $opens_1->open_ent) <!--ok -->
                    @if ($openCount = 1)
                ...
                <td id="sourceTd">
                    ...
                    {{-- Store the closing value in a PHP variable --}}
                    @php
                    $closing_value_1 = $totalForRow - $totalOrderQuantityForRow;
                    ${"closing_".$i1} = $closing_value_1;
                    @endphp

                    {{-- Calculate and output the closing value --}}
                    @if((empty($total_opening_stock_pass) || empty($total_order_quantity_pass)) || (($total_opening_stock_pass == '0') || ($total_order_quantity_pass == '0')))
                    {{-- Calculate closing value and assign it to a variable --}}
                    @php
                    $previous_closing = ${"closing_".($i1 - 1)} ?? 0;
                    ${"closing_".$i1} = $closing_value_1 + $previous_closing;
                    @endphp
                    {{ ${"closing_".$i1} }}
                    {{-- @php
            $close_val = ${"closing_".($i1 - 1)} ?? 0;
            $closing_value_1 = $closing_value_1 +  $close_val;
        @endphp --}}
                    @else
                    {{ $totalForRow - $totalOrderQuantityForRow }}
                    @php
                    number_format(${"closing_".$i1} = $closing_value_1,2);
                    @endphp
                    @endif

                    {{--
        <input type="text" id="closing_{{ $i1 }}" value="{{ $totalForRow - $totalOrderQuantityForRow }}">
                    (If you're planning to use an input element for the closing value, uncomment the line above)
                    --}}
                </td>
                ...
                <td>
                    ...
                    {{ number_format(${"closing_".($i1 - 1)}  - ${"opening_".($i1)} ?? 0 ,2)}}
                    @else
                    @php
                    echo "0";
                    @endphp
