        ...
        <div class="table-responsive">
            ...
            <tbody>
                ...
                @foreach($result as $results)
                    @php
                        $dates = date('Y-m-d');
                        $orderDate = new DateTime($results->dispatch_date);
                        $dispatchDate = new DateTime($dates);

                        $interval = $orderDate->diff($dispatchDate);
                        $daysDifference = $interval->days;
                    @endphp
                    @if($results->id != $results->tn)
                        <tr>
                            <td>{{ $i }}</td>
...
function printTable() {
    ...
    const headerRow1 = ['SuperLuber'];
    const headerRow2 = ['Not PAY-{{$from_date}} TO {{$to_date}}'];
