...
class SalesReturnD2CController extends Controller
{
    ...
    public function retrieve_sub($main_id,$sub_id)
    {
        if($sub_id=='')
        {
            ...
            $tb1=DB::select('select id,order_date_sub,time_sub,(select tally_no from ' . $SalesOrderDeliveryMain_tb . ' where id=' . $sub_tb . '.tally_no) as tally_no,batch_no,(select return_type from ' . $ReturnCreation_tb . ' where id=' . $sub_tb . '.return_type_id) as return_type_id,(select item_name from ' . $ItemCreation_tb . ' where id=' . $sub_tb . '.item_creation_id) as item_creation_id,
        ...
        {return SalesReturnD2CSub::where('id',$sub_id)->where(function($query){$query->where('delete_status', '0')->orWhereNull('delete_status');})->get(['id','order_date_sub','time_sub','tally_no','batch_no','return_type_id','group_creation_id','item_creation_id','order_quantity','pieces_quantity','item_property','item_weights','item_price','total_amount'])->first();}
    }
