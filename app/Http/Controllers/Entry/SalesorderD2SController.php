...
class SalesorderD2SController extends Controller
{
    ...
    public function retrieve_sub($main_id, $sub_id)
    {
        ...
        if ($sub_id == '') {
            ...
            $tb1 = DB::select('select id,status_check,order_date_sub,arriving_time_sub,closing_time_sub,
            ...
            (select secondary_image from ' . $ShopCreation_tb . ' where id=' . $sub_tb . '.shop_creation_id) as secondary_image,
            (select shop_name from ' . $ShopCreation_tb . ' where id=' . $sub_tb . '.shop_creation_id) as shop_creation_id,
            (select item_properties_type from ' . $ItemPropertiesType_tb . ' where id=' . $sub_tb . '.item_property) as item_property,
            (select item_liters_type from ' . $ItemLitersType_tb . ' where id=' . $sub_tb . '.item_weights) as item_weights,current_stock,order_quantity,pieces_quantity,item_price,total_amount from ' . $sub_tb . ' where sales_order_main_id=' . $main_id . ' and (delete_status=0 or delete_status is null)');

            return json_decode(json_encode($tb1), true);
        }
        else {
            return SalesOrderD2Ssub::where('sales_order_d2s_sublist.id', $sub_id)->join('sales_order_d2s_main', 'sales_order_d2s_main.id', '=', 'sales_order_d2s_sublist.sales_order_main_id')->where(function ($query) {
                $query->where('sales_order_d2s_sublist.delete_status', '0')->orWhereNull('sales_order_d2s_sublist.delete_status');
            })->get(['sales_order_d2s_main.dealer_creation_id','sales_order_d2s_sublist.id','sales_order_d2s_sublist.status_check','sales_order_d2s_sublist.order_date_sub','sales_order_d2s_sublist.arriving_time_sub','sales_order_d2s_sublist.closing_time_sub','sales_order_d2s_sublist.group_creation_id','sales_order_d2s_sublist.market_creation_id','sales_order_d2s_sublist.shop_creation_id','sales_order_d2s_sublist.item_creation_id','sales_order_d2s_sublist.short_code_id','sales_order_d2s_sublist.current_stock','sales_order_d2s_sublist.order_quantity','sales_order_d2s_sublist.pieces_quantity','sales_order_d2s_sublist.item_property','sales_order_d2s_sublist.item_weights','sales_order_d2s_sublist.item_price','sales_order_d2s_sublist.total_amount'])->first();
        }
    }
    public function visitors_retrieve($id)
    {
        if($id=='')
        {return VisitorCreation::select('id','d2s_id','order_date','order_no','dealer_id','visitor_dis','visitor_name','sales_exec','mobile_no','description','address','image_name')->orderBy('id')->get();}
        else
        {return VisitorCreation::select('id','d2s_id','order_date','order_no','dealer_id','visitor_dis','visitor_name','sales_exec','mobile_no','description','address','image_name')->where('id','=',$id)->get();}
    }
    public function visitors_sublist_retrieve($d2s_id,$order_date,$order_no,$sales_exec)
    {
        ...
        else
        {
            ...
            ->select('vs.id', 'vs.d2s_id', 'vs.order_date', 'vs.order_no', 'vs.dealer_id', 'vs.visitor_dis', 'vs.visitor_name', 'vs.sales_exec', 'vs.mobile_no', 'vs.description', 'vs.address', 'vs.image_name', 'dc.dealer_name')
            ->leftJoin('dealer_creation as dc', 'vs.dealer_id', '=', 'dc.id')
            ->orderBy('vs.id')
            ->where('order_date', '=', $order_date)
            ->where('order_no', '=', $order_no)
            ->where('sales_exec', '=', $sales_exec)
            ->get();

        return $VisitorCreation;
        }
    }
    public function db_cmd(Request $request)
    {
        ...
        if ($action == 'insert') {
            ...
            $visitor = VisitorCreation::where('d2s_id',$request->input('id'))->first();

            if ($visitor) {
                $visitor->delete();
            }

            $Sales_sub = SalesOrderD2Ssub::where('sales_order_main_id',$request->input('id'))->first();
            ...
            //             'entry_date'  => Carbon::now(),
            //             'order_no' => $order_no,
            //             'stock_entry_date' => $request->input('order_date'),
            //             'dealer_creation_id' => $request->input('dealer_creation_id'),
            ...
            //         $tb->item_creation_id = 0;
            //     }
            //     if($request->input('short_code_id')){
            //         $tb->short_code_id = $request->input('short_code_id');
            //     }else{
            //         $tb->short_code_id = 0;
            //     }
            ...
            //     }
            //     if($request->input('order_quantity')){
            //         $tb->current_stock = $request->input('order_quantity');
            //     }else{
            //         $tb->current_stock = 0;
            //     }
            //     if($request->input('pieces_quantity')){
    ...
    // return $sales_order_d2s_main;
            ...
            $sales_order_d2s_main = $this->retrieve_main($main_id, '');
            ...
            if ($main_id != "" && $sub_id == "") {
                ...
                $sales_order_d2s_sub = SalesOrderD2Ssub::select('market_creation_id')
                ...
                if($sales_order_d2s_sub){
                    $market_creation_id = $sales_order_d2s_sub->market_creation_id;
                }else{
                    $market_creation_id = 0;
                }
                $market_id = $dealer_creation ? $dealer_creation->area_id : null;
                ...
                foreach ($market_ids as $marketId) {
                    ...
                    if ($area_name) {
                        $market_creation[] = $area_name; // Add the entire MarketCreation object to the array
                        $marketId_s[] = $marketId;
                    }
                }
            ...
            } elseif ($main_id = "" && $sub_id = "") {
                $sales_order_d2s_sub = $this->retrieve_sub($main_id, $sub_id);
                $dealer_creation_id = $sales_order_d2s_sub->dealer_creation_id;
                $market_creation_id = $sales_order_d2s_sub ->market_creation_id;
                $dealer_creation = DealerCreation::find($dealer_creation_id);
            ...
            ->where('id', $dealer_creation_id)->where('status', '1')
            ->get();
        ...
        else if ($action == 'visitors_insert')
        {
            ...
            $tb1 = SalesOrderD2SMain::find($request->input('d2s_id'));
            $tb1->dealer_creation_id = $request->input('dealer_id');
            $tb1->save();
        }
