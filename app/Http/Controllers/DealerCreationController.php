...
class DealerCreationController extends Controller
{
    ...
    public function shop_retrieve($id)
    {
        ...
        {return ShopCreation::select('id','shop_name','shop_type_id','beats_id','dealer_id','mobile_no','whatsapp_no','address','gst_no','image_name','secondary_image','language')->orderBy('id')->where('id','=',$id)->get();}
    }
            ...
            foreach ($market_ids as $marketId) {
                ...
                if ($area_name) {
                    ...
                    $digits = str_split($marketId);
                        ...
                        $language = ShopCreation::where('beats_id', '=', $marketId)
                            ->where('dealer_id', '=', $dealer_id)
                            ->where(function ($query) {
                                $query->where('delete_status', '0')->orWhereNull('delete_status');
                            })
...
// return $marketId;
        ...
        else if($action=='create_form')
        {
            ...
            $manager = MarketManagerCreation::select('id', 'manager_name')
                ->where(function ($query) {
                    ...
                    ->where('status1','=','0')->orWhereNull('delete_status');
                 })
                ->orderBy('manager_name')
                ->get();

            $state_name = StateCreation::select('id', 'state_name')
                ->where('state_name', '!=', '')->where(function ($query) {
                    $query->where('delete_status', '0')->orWhereNull('delete_status');
                })
                ->orderBy('state_name')
                ->get();
            ...
            $district_name = DistrictCreation::select('id', 'district_name')
                ->where('district_name', '!=', '')->where(function ($query) {
                    $query->where('delete_status', '0')->orWhereNull('delete_status');
                })
                ->orderBy('district_name')
                ->get();
            ...
            $area_name = MarketCreation::select('id', 'area_name')
                ->where('area_name', '!=', '')->where(function ($query) {
                    $query->where('delete_status', '0')->orWhereNull('delete_status');
                })
                ->orderBy('area_name')
                ->get();
        ...
        else if($action=='update_form')
        {
            ...
            $dealer_ids = $request->input('id');
                ...
                ->where('id', $dealer_ids)->where(function ($query) {
                    $query->where('delete_status', '0')->orWhereNull('delete_status');
                })
                ->first();
            ...
            $state_name = StateCreation::select('id', 'state_name')
                ...
                $district_name = DistrictCreation::select('id', 'district_name')
                ->where('state_id', '=', $dealer_drop_id)->where(function ($query) {
                    $query->where('delete_status', '0')->orWhereNull('delete_status');
                })
                ->orderBy('id')
                ->get();
                ...
                $area_name = MarketCreation::select('id', 'area_name')
                ->where('district_id', '=', $dealer_query->district_id)->where(function ($query) {
                    $query->where('delete_status', '0')->orWhereNull('delete_status');
                })
                ->orderBy('id')
                ->get();
