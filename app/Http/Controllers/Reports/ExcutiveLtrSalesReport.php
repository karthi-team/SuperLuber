...
class ExcutiveLtrSalesReport extends Controller
{
    public function excutive_report()
    {
        ...
        ->where('delete_status', '!=', '1')->where('status', '0')
        ...
        if ($action == 'view') {
            ...
            ->select(
                ...
                'sales_order_d2s_main.order_date',
            ...
            ->groupBy('area_creation.area_name', 'sales_order_d2s_main.order_date', 'sales_ref_creation.sales_ref_name', 'dealer_creation.dealer_name', 'market_manager_creation.manager_name');
        ...
        $results = $query->limit(25)->get();
            ...
            ->where('manager_id', $manag)->where('status', '0')->where(function ($query) {
                $query->where('delete_status', '0')->orWhereNull('delete_status');
            })
            ->get();
            ...
            ->where('sales_rep_id', $sales_exec)->where('status', '1') ->where(function ($query) {
                $query->where('delete_status', '0')->orWhereNull('delete_status');
            })
            ->get();
