...
class DealerOrderReport extends Controller
{
    public function dealer_order_report()
    {
        ...
        ->where('status', '1')->where(function($query){$query->where('delete_status', '0')->orWhereNull('delete_status');})->orderBy('dealer_name')->get();
     ...
     public function db_cmd(Request $request)
    {
        ...
        ->get();
            ...
            $query = DB::table('sales_order_d2s_main as sodm')
                ...
                ->where(function ($query) {
                    $query->whereNull('sods.delete_status')
                          ->orWhere('sods.delete_status', 0);
                });
...
//$test = $query;
 ...
 //return $records;
            ...
            $sales_ref_name = SalesRepCreation::select('id', 'sales_ref_name')->where('manager_id', $manager_id)
            ->where('status', '0')           
            ->where(function ($query) {
                $query->where('delete_status', '0')->orWhereNull('delete_status');
            ...
            $dealer_creation=DealerCreation::select('id','dealer_name')
            ->where('status', '1')->where('sales_rep_id', $sales_ref_id)->where('manager_name', $manager_id)->where(function($query){$query->where('delete_status', '0')->orWhereNull('delete_status');})->orderBy('dealer_name')->get();
