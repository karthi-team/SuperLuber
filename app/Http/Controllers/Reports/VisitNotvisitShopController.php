...
class VisitNotvisitShopController extends Controller
{
    public function visit_shop_Report()
    {
        ...
        $manager_names = MarketManagerCreation::select('id', 'manager_name')->where('status1', '0')->where(function ($query) {
            $query->where('delete_status', '0')->orWhereNull('delete_status');
        ...
        return view('Reports.visit_nt_visit_shop.admin', ['order_no_list' => $order_no_list, 'market_creation' => $market_creation, 'dealer_creation' => $dealer_creation, 'shop_creation' => $shop_creation, "sales_name" => $sales_name,'item_namess' =>$item_namess,'manager_names'=>$manager_names, 'group_creation'=>$group_creation]);
    }
    public function retrieve($from_date_1, $sales_exec, $dealer_creation_id_1, $manager_na, $order_shop, $order_shop1, $item_name_1, $group_id, $item_creation_ids)
    {
        ...
        $query->select(array_merge([
            ...
            DB::raw('GROUP_CONCAT(DISTINCT shop_creation.shop_name) AS shop_name'),
            DB::raw('GROUP_CONCAT(DISTINCT CASE WHEN sublist.status_check = "No" THEN shop_creation.shop_name END) AS shop_names'),
            DB::raw('GROUP_CONCAT(DISTINCT CASE WHEN sublist.status_check = "Yes" THEN shop_creation.shop_name END) AS shop_names_1'),
        ...
        $query ->where('market_manager_creation.status1', '0')
                ->where('sales_ref_creation.status', '0')
                ->where('dealer_creation.status', '1');
      ...
      if ($action == 'retrieve') {
       ...
       $item_names_1= $request->input('item_name_1');
        ...
        // $itemCreationShortCount = $item_creation_short->count();
            ...
            return view('Reports.visit_nt_visit_shop.list', [
            ...
            $dealer_name = SalesRepCreation::select('id', 'sales_ref_name')->where('status', '0')->where(function ($query) {
                $query->where('delete_status', '0')->orWhereNull('delete_status');
            })
