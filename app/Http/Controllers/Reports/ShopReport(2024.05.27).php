...
class ShopReport extends Controller
{
    ...
    public function db_cmd(Request $request)
    {
        ...
        if ($action == 'retrieve') {
            ...
            $shop_list1 = ShopCreation::selectRaw("$shop_tb.id as shop_id,$shop_tb.shop_name,$dealer_tb.place,$shop_tb.address,$shoptype_tb.shops_type")->leftJoin($dealer_tb, "$dealer_tb.id", "$shop_tb.dealer_id")->leftJoin($shoptype_tb, "$shoptype_tb.id", "$shop_tb.shop_type_id")->whereRaw("($shop_tb.delete_status='0' or $shop_tb.delete_status is null)" . ((count($cond1) > 0) ? " and (" . implode(" and ", $cond1) . ")" : ""))->get();
            $result = [];
            if ($shop_list1 != null) {
                $d2s_main_ids = [];
                ...
                foreach ($shop_list1 as $shop_list2) {
                    ...
                    if ((count($d2s_main_ids) > 0) && (count($d2s_sub_ids) > 0)) {
                        $sales_sub1 = SalesOrderD2Ssub::selectRaw("$d2ssub_tb.*,$d2smain_tb.order_date as order_date_main")->leftJoin($d2smain_tb, "$d2smain_tb.id", "$d2ssub_tb.sales_order_main_id")->whereRaw("$d2ssub_tb.shop_creation_id='$shop_id'$ch_cond")->orderBy("$d2ssub_tb.sales_order_main_id", 'asc')->get();
                        if ($sales_sub1 != null) {
                            foreach ($sales_sub1 as $sales_sub2) {
                                if (!array_key_exists($sales_sub2['sales_order_main_id'], $main_datas)) {
                                    $main_datas[$sales_sub2['sales_order_main_id']] = ['date' => [], 'yes' => []];
                                }
                                if (!in_array($sales_sub2['order_date_main'], $main_datas[$sales_sub2['sales_order_main_id']]['date'])) {
                                    $main_datas[$sales_sub2['sales_order_main_id']]['date'][] = $sales_sub2['order_date_main'];
                                }
                                if ($sales_sub2['status_check'] == 'Yes') {
                                    if (!in_array($sales_sub2['order_date_main'], $main_datas[$sales_sub2['sales_order_main_id']]['yes'])) {
                                        $main_datas[$sales_sub2['sales_order_main_id']]['yes'][] = $sales_sub2['order_date_main'];
                                    }
                                }
                    ...
                    if (count($main_datas) > 0) {
                        foreach ($main_datas as $main_ids1 => $main_datas1) {
                    ...
                    if ($ch_row) {
                        ...
                        $Productive_Not_Purchase = ($No_of_Visits - $Productive_Purchase);
                        $result[] = [
            ...
            $dealer_creation = DealerCreation::select('id', 'dealer_name')->where('sales_rep_id', $sales_ref_id)->where('manager_name', $manager_id)->where('status',  '1')->where(function ($query) {
                $query->where('delete_status', '0')->orWhereNull('delete_status');
