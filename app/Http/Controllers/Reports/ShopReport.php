        ...
        if ($action == 'retrieve') {
            ...
            if ($shop_list1 != null) {
                ...
                if (!empty($sales_ref_id)) {
                    $d2s_main_ids = SalesOrderD2SMain::whereRaw("(sales_exec='$sales_ref_id') and (delete_status='0' or delete_status is null) and status='1'")->distinct()->pluck('id')->toArray();
                } else {
                    $d2s_main_ids = SalesOrderD2SMain::whereRaw("(delete_status='0' or delete_status is null) and status='1'")->distinct()->pluck('id')->toArray();
                }
                $d2s_sub_ids = [];
                if ((!empty($from_date)) || (!empty($to_date)) || (!empty($group_id)) || (!empty($item_id))) {
                    ...
                    $d2s_sub_ids = SalesOrderD2Ssub::whereRaw("(delete_status='0' or delete_status is null) and ".implode(' and ', $cond))->distinct()->pluck('id')->toArray();
                } else {
                    $d2s_sub_ids = SalesOrderD2Ssub::whereRaw("(delete_status='0' or delete_status is null)")->distinct()->pluck('id')->toArray();
                }
                $ch_cond = "";
                ...
                foreach ($shop_list1 as $shop_list2) {
                    ...
                    if ((count($d2s_main_ids) > 0) && (count($d2s_sub_ids) > 0)) {
                        $sales_sub1 = SalesOrderD2Ssub::selectRaw("sales_order_main_id,order_date,status_check,order_quantity")->whereRaw("(delete_status='0' or delete_status is null) and shop_creation_id='$shop_id'$ch_cond")->orderBy("sales_order_main_id", 'asc')->get();
                        if ($sales_sub1 != null) {
                    ...
                    if ($ch_row) {
                        $last_dt1 = SalesOrderD2Ssub::selectRaw('max(order_date) as last_dt')->whereRaw("(delete_status='0' or delete_status is null) and shop_creation_id='$shop_id'")->first();
                        $last_dt = "";
                        if ($last_dt1 != null) {
